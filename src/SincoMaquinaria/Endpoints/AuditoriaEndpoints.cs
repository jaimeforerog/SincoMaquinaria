...
public static class AuditoriaEndpoints
{
    ...
    private static async Task<IResult> ListarEventos(
        ...
        int pageSize = 50)
    {
        IQueryable<SincoMaquinaria.Domain.Projections.RegistroAuditoria> query =
            session.Query<SincoMaquinaria.Domain.Projections.RegistroAuditoria>();
        ...
        if (fechaFin.HasValue)
        {
            ...
            query = query.Where(e => e.Fecha <= new DateTimeOffset(endOfDay));
        }

        if (!string.IsNullOrEmpty(modulo))
            query = query.Where(e => e.Modulo == modulo);

        if (!string.IsNullOrEmpty(tipo))
            query = query.Where(e => e.TipoEvento == tipo);

        if (!string.IsNullOrEmpty(usuario))
            query = query.Where(e => e.UsuarioNombre != null && e.UsuarioNombre.Contains(usuario, StringComparison.OrdinalIgnoreCase));
